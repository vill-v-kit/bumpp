//! recursive 能力矩阵：链上清单模式表聚合（opt-in； 底座化）。

#[test]
fn recursive_manifest_globs_cover_all_ecosystems() {
  // 链上模式表（opt-in）：node 8 种 manifest + Cargo.toml；
...
fn default_file_patterns_are_root_level_chain_union() {
  // files 为空时的默认清单 = 链上 manifest basenames 根级并集
...
fn default_file_patterns_recursive_upgrades_to_tree_globs() {
  // recursive 默认清单 = 同一份 basename 表的 `**/` 整树收集模式

...
fn env_hit_reports_exact_variable_name() {
  ...
  let got = resolve_token_sourced(Provider::Github, &BTreeMap::new(), &both, &no_gh).unwrap();
  ...
  let got =
    resolve_token_sourced(Provider::Github, &BTreeMap::new(), &only_github, &no_gh).unwrap();
  ...
  let got = resolve_token_sourced(Provider::Gitlab, &BTreeMap::new(), &env, &no_gh).unwrap();
...
fn gh_cli_fallback_reports_gh_source() {
  ...
  let got = resolve_token_sourced(Provider::Github, &BTreeMap::new(), &no_env, &gh).unwrap();
...
fn empty_store_value_falls_through_and_reports_real_source() {
  ...
  let got =
    resolve_token_sourced(Provider::Github, &tokens(&[("github", "")]), &env, &no_gh).unwrap();

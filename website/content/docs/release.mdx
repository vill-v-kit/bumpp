...
`host` 保留端口与路径（兼容 relative-url-root 部署）；显式 `http://` 原样保留，覆盖内网纯 HTTP 实例。
...
录入的 `--host` 与配置的 `host` 都不要求写法精确一致：`gitlab-a.com`、`https://gitlab-a.com/`、`HTTPS://GitLab-A.com` 归一为同一条目。查看与删除同样按实例进行：
...
| 2 | provider 级 `gitlab` 条目（`vbumpp token set gitlab` 录入——存量配置零感知回落） |
| 3 | `GITLAB_TOKEN` 环境变量 |

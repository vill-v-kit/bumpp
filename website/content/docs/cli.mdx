...
生成 `.vbumpprc` 配置文件对应的 JSON Schema——编辑器靠它在编写配置时给出补全与报红（JSON / JSONC / TOML 三种格式共用同一份，用法见[配置文件参考](/docs/configuration)）。
...
`--project` 与 `--global` 互斥，不能同时给；不给 `--write` 时落点参数无效，一律打印到 stdout。生成的 schema 与当前安装的 vbumpp 版本一致——离线或内网环境可以把写出的文件提交进仓库，配置里改用相对路径引用它。

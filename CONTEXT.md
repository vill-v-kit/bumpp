...
**Core**:
纯 Rust + napi-rs 实现的版本引擎包 `@vill-v/bumpp-core`（`npm/bumpp-core`），对外提供三个与上游 bumpp v11 兼容的 API：`versionBump` / `versionBumpInfo` / `loadBumpConfig`。进度为 Rust 内置打印（ADR-0002），`ProgressEvent` 不向 Node 层导出。

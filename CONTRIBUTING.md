...
工具链维护：rust 升版 = 改 `mise.toml` 版本号 + `mise lock`。hk 升版 = `mise.toml` 版本号 + 重下 hk@对应版本 zip 覆盖根 `pkl/`（vendored schema）+ `mise lock` 三处同步。macOS 主机临时复现 arm64 交叉产物，不改 `mise.toml`，用显式 spec 现取：

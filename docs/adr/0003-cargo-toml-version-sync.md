...
- **显式纳入 files**：Cargo.toml 进入发版清单靠显式配置（本仓 `vbumpp.config.ts` 列明各 rust crate 的 Cargo.toml——当前为 `crates/bumpp-core`、`crates/version-files`、`npm/bumpp-core` 三处），不做 recursive 自动收集——避免误伤其他仓库中无关的 Cargo.toml。
...
- **本仓配置**：`vbumpp.config.ts` 的 `bumpp.files` 列明 `crates/bumpp-core`、`crates/version-files`、`npm/bumpp-core` 三处 Cargo.toml（与默认 `package.json` / `package-lock.json` 合并去重）。

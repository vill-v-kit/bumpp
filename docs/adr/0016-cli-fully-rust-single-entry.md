...
- **Release 重试**：`vbumpp release <version>` 从 changelog 文件提取目标版本节（`-o` 优先，未给时回落配置 `changelog.output`，内建默认 `CHANGELOG.md`——ADR-0039），并复用现有 token、仓库解析和 provider 创建机制。输入版本接受 `5.1.0` 与 `v5.1.0`。
...
- **配置覆盖**：默认 bump 命令仅在用户提供 files 时注入 `files` override，避免空数组覆盖配置文件；recursive 恒注入，changelog output 仅在显式 `-o` 时注入（未给时配置优先，ADR-0039）。

...
网站安装指南承诺 npm 与 crates.io；当前仓库有 11 个 npm manifest（5 个用户包、1 个 core 包、5 个平台包）以及 2 个公开 crate（`vbumpp-core`、`vbumpp`）。musl 平台包纳入后，npm 发布规模为 13 个包；`napi/bumpp-core` 内部 Cargo crate 不属于这 2 个 registry crate。
...
- npm 发布先下载 build 产物，将 7 个 `.node` 注入平台包并将 `index.d.ts` 注入 core；断言 tag 与根 workspace 版本一致，构建用户包，执行 pack/publint，再由 `pnpm publish -r --no-git-checks` 按拓扑发布。发布集合覆盖 13 个 npm 包，website 与根 workspace 的 private 包自动跳过。

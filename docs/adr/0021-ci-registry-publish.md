...
- **publish-npm**：下载 build job 的 5 个 `.node` artifact 注入各平台包目录（`index.d.ts` 同车捎带——`@vill-v/bumpp-core` 的 `types` 入口为 napi build 生成物、不在 git 中，缺它 publint 报错）→ 断言 tag 版本 == 根 workspace 版本 → `pnpm -r pack` + publint 前置验证 → **skip-if-published 守卫**（`npm view <pkg>@<version>` 已存在即跳过）→ `pnpm publish -r --no-git-checks`。`workspace:*` 协议转换与拓扑序（平台包 → core → 用户包）由 pnpm 原生处理；`website` 为 private 自动跳过。

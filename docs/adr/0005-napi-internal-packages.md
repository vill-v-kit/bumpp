...
- **发布判别（要发版 → npm/）**：让 `@vill-v/bumpp-core` 这种用户从不直接安装的绑定包与 `@vill-v/bumpp` 这种面向用户的 CLI 混居 `npm/`，目录名与受众错位——拒绝。
- **产物形态判别（含 .node 产物 → napi/）**：当前包集合上结果与受众判别相同，但依据的是产物而非意图；未来出现纯 JS 内部包（如共享脚本包）会被误判到面向用户区——拒绝，但承认其为受众判别的近似。
- **规则先行、搬迁另立票**：中间态约定与现实脱节（约定文档说一套、示例包做另一套）——拒绝，同变更原子落地。
...
- `npm/bumpp-core` 归位 `napi/bumpp-core`，目录示例与约定一致。

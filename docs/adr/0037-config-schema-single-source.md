...
配置形状此前散落多处——顶层键白名单 const、默认值表、changelog 手写解析、npm 包手写 TS interface——彼此靠人工同步；且文件层只校验键名不校验类型，类型不符静默回落默认（如 `files: "x"` 致空跑）。决定以 Rust 结构体为单一事实源，机械导出 JSON Schema 与 TS 类型；文件层升级为键名+类型双重校验；schema 经 SchemaStore、npm 包副本与 `vbumpp schema` 子命令分发，让编辑器提示与配置验证工程化。
...
- **分发与防腐**：schema JSON 提交进仓库两处（npm/bump 包内副本、website 静态导出），由 `scripts/` TS 脚本调 `vbumpp schema` 再生，ci.yml 加漂移校验腿（重跑 diff，不一致即红，同 demo-cast 模式）；文档站 Pages URL 为规范地址，向 SchemaStore 提交收录（fileMatch 覆盖 `.vbumpprc.{json,jsonc,toml}` 三格式，TOML 侧由 Taplo 消费同一目录）。三格式共用同一份 schema，不维护 TOML 单独版本。
...
- SchemaStore 收录是对外承诺：Pages URL 长期稳定，内容随发版更新、地址不变。

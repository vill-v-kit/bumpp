...
- **再拆触发条件**：出现第二消费方时，按 ADR-0004 同款流程拆 `crates/package-manager`——逻辑已内聚单模块，届时拆分是纯移动，成本极低。

## Considered Options

- **拆 `crates/package-manager` 独立 crate**：兑现独立测试与上游演进集中点，但唯一消费方下收益主要是账面的；代价是 workspace 成员 +1、发版版本线 +1，且与 ADR-0004"不为唯一消费方提前拆包"的原则直接冲突——拒绝，留触发条件。

...
v6.1.0 发版接连两起事故。其一:COL-76 提交前未跑 `cargo fmt`,而 CI 仅在 `v*` tag 推送时触发(ADR-0021「tag 推送即授权」),fmt/clippy/test 的唯一运行时机就是发版当场——tag CI 首跑即挂,修复要移 tag 重推。其二:musl ×2 新平台包的 OIDC 首发 404(见 ADR-0021 决策④首发仪式),漏做仪式导致 publish-npm 拓扑序中段失败、4/13 包已上架的部分发布态。本 ADR 记录事故复盘后的质量门前移与工具链钉版决策;首发仪式流程本身不变(ADR-0021/0029 已载),「矩阵扩张 → 必做仪式」的触发绑定缺口由 npm-publish.mjs 前置检测闭环(见 Consequences)。
...
- 矩阵扩张 → 首发仪式的触发绑定已闭环:npm-publish.mjs 增「全新包名」前置检测——对守卫放行的包逐个查包级文档,包名 404(从未上架)时在 CI 非 dry-run 整体拦停(exit 2,与守卫原子性同义)并输出仪式指引;本地手动首发是仪式本身,只警告不拦;包级查询失败同守卫语义 exit 2,不在未知态放行半个计划。未来扩 target 漏做首发时,publish-npm 在上传任何包之前硬失败,不再出现拓扑序中段 404 的部分上架态。

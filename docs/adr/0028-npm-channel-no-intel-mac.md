...
- npm 平台包矩阵固定为 5 targets：`darwin-arm64`、`linux-x64-gnu`、`linux-arm64-gnu`、`win32-x64-msvc`、`win32-arm64-msvc`，不含 darwin-x64。【修订：报错形态条款反转】loader 曾以自研的 "Supported platforms: …" 硬错误兜底；loader 回归 napi-rs 官方生成物后（ADR-0033），Intel Mac 上为生成物的 npm#4828 标准报错，不再列出平台清单——矩阵排除 darwin-x64 的决策本身不变。
...
- Intel Mac 用户在 npm 安装阶段不报错（平台包是 optionalDependencies），首次运行时才遇 loader 加载失败报错（ADR-0033 后为 napi-rs 生成物标准文案，不含平台清单——已接受的 UX 退化）；文档说明是唯一的软着陆点，必须保持准确。
- 若未来 Intel Mac 诉求回潮（企业存量设备等），重开评估的触发条件是出现真实用户反馈，而不是预防性补腿。
- 参考锚点：ADR-0025（binstall 渠道的矩阵与回退语义）、ADR-0005（napi 平台包分发机制）、ADR-0033（loader 回归生成物，报错形态反转的完整上下文）。

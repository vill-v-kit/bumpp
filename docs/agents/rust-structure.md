...
`src/` 顶层平铺模块不设数量上限，但新增前先评估能否并入既有子域（`cli/`、`changelog/`、`plugins/`、`release/` 等）；确需新顶层时在 PR 描述里说明理由。设数字上限会逼出凑数分组，所以是行为约束不是数字约束。
...
- `tests/` 目录结构与 `src/` 模块一一镜像；多文件测试域用 `main.rs` + 子模块组织（先例：`tests/plugins/`、`tests/release/`）。
